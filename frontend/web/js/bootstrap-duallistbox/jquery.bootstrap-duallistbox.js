...
;(function ($, window, document, undefined) {
  ...
  var pluginName = 'bootstrapDualListbox',
    defaults = {
      ...
      moveSelectedLabel: 'Переместить',
      moveAllLabel: 'Переместить все',
      removeSelectedLabel: 'Удалить выбранное',
      ...
      infoText: 'Показано {0}',                                                        // text when all options are visible / false for no info text
      infoTextFiltered: '<span class="label label-warning">Отфильтровано</span> {0} из {1}', // when not all of the options are visible due to the filter
      infoTextEmpty: 'Список пуст',                                                        // when there are no options present in the list
  ...
  BootstrapDualListbox.prototype = {
    ...
    setBootstrap2Compatible: function(value, refresh) {
      ...
      if (value) {
        ...
        this.container.find('.moveall > i, .move > i').removeClass('fas fa-arrow-right').addClass('fas fa-arrow-right');
        this.container.find('.removeall > i, .remove > i').removeClass('fas fa-arrow-left').addClass('fas fa-arrow-left');
      } else {
